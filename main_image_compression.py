...
def main_image_compression():
    ...
    list_of_keywords, list_of_shortened_words, radius_len, group_number_len = affiliation_to_groups(
        list_sorted_by_color,
        list_of_pixels.__len__(),
        length_word)

    # Save as "png" format to reduce image size
    image_size = (int(ceil(image_size[0] / sqrt(length_word / (group_number_len + radius_len)))),
                  int(ceil(image_size[1] / sqrt(length_word / (group_number_len + radius_len)))))
